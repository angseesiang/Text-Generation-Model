...
Minimal GPT-2 text generation with:
- Class API (TextGenerationModel)
- CLI: --prompt "..." [--sample --temperature 0.9 --top_p 0.95 ...]
- Save/load to a local ./model dir (customizable via --save_dir)
- Backward-compat aliases: save_model(), load_model() for existing tests
"""
...
class TextGenerationModel:
    def __init__(
        ...
        Args:
            ...
            trust_remote_code: passed to transformers; usually keep False.
        """
    ...
    # --- Backward-compat aliases for your unit tests ---
    def save_model(self, save_dir: str = "model") -> None:
        """Alias for .save() to keep tests happy."""
        return self.save(save_dir)

    @classmethod
    def load_model(cls, load_dir: str = "model", device: Optional[str] = None) -> "TextGenerationModel":
        """Alias for .load() to keep tests happy."""
        return cls.load(load_dir=load_dir, device=device)
...
def main() -> None:
    ...
    # Reduce TF log noise if tf-keras is present
